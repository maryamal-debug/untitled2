# Toters App

An application that displays all restaurants near or far with a description of them and their menu to order from them at the lowest cost of delivery with great offers
## Badges


[![MIT License](https://img.shields.io/badge/Dart-2.1-blue)](https://choosealicense.com/licenses/mit/)
[![GPLv3 License](https://img.shields.io/badge/Flutter-3.3.1-9cf)](https://opensource.org/licenses/)
[![AGPL License](https://img.shields.io/badge/Node%20js-16.17-brightgreen)](http://www.gnu.org/licenses/agpl-3.0)


![Logo](data:image/png;base64,)


## Pub_dev backages

#### backages


| backages name    | version     | Description                |
| :--------        | :-------    | :------------------------- |
| `carousel_images`| `^1.1.2`    ||Carousel of images with reality (parallax) effect. Support Network and Asset Images.
| `google_nav_bar`| `^^5.0.6`    ||A modern google style nav bar which could be use as a bottom navigation bar or tabbar, the design strictly follows the cuberto UI/UX on dribbble.



## Screenshots

![App Screenshot](https://k.top4top.io/p_2452c8ybs1.png)
![App Screenshot](https://l.top4top.io/p_2452uk3k12.png)
![App Screenshot](https://a.top4top.io/p_2452tpc633.png)
![App Screenshot](https://b.top4top.io/p_2452j2xg64.png)
![App Screenshot](https://c.top4top.io/p_2452j0iyo5.png)


## Installation


```bash
  1 - Download Project
```
```bash
  2 - Unzip Project
```
```bash
  1 - Open Android Studio
```

## Deployment

First time run this commend in terminal

```bash
  flutter pub get
```


## Authors

- [@Maryam Ali](https://github.com/maryamal-debug)


## License

[DART](https://dart.dev/)
[Flutter](https://flutter.dev/?gclid=CjwKCAjwg5uZBhATEiwAhhRLHmJbhLd-xa8jymLRIbIzxq1UdoJZSF0tc2qXw-vdRMphvvF8fPUsOhoCCmsQAvD_BwE&gclsrc=aw.ds)
[android studio](https://developer.android.com/studio?gclid=CjwKCAjwg5uZBhATEiwAhhRLHv-Vji4x08ZRSxzhF1caxS489BLDEOEH11BG92-gMX3WMYx3mjVuuRoCAIAQAvD_BwE&gclsrc=aw.ds)
[nodejs](https://nodejs.org/en/)
[visual studio code](https://code.visualstudio.com/)
